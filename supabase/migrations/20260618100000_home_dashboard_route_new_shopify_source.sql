-- Home dashboard bucket-routing fix per C Chat handover v4 §7.
--
-- After the campaign_orders retirement, historic_orders gained two
-- new source_platform values that didn't exist when the dashboard
-- was last written:
--   * 'shopify'  - the ISOT 2022 post-campaign Shopify store
--                  (Feb-Mar 2022, ~4,215 orders / $349k). Belongs
--                  in the Shopify column, not Other Sources.
--   * 'crowdox'  - pre-existing crowdfunding platform (~1,677 orders
--                  / $167k). Already routes correctly to Other
--                  Sources via the NOT IN exclusion list.
--
-- This migration widens the Shopify filter to
--   source_platform IN ('shopify', 'shopify_legacy')
-- and adds 'shopify' to the Other Sources NOT IN exclusion list.
-- KPI math elsewhere unchanged.

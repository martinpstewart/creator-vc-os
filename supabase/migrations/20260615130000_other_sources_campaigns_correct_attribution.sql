-- Other Sources campaigns list: fix attribution path.
--
-- The prior migration joined historic_orders to `campaign_orders` to
-- attribute orders to campaigns. That table only carries rows for live
-- Shopify orders — Indiegogo, Kickstarter, and Wix orders have no
-- presence there, so 3 of the 4 historical campaigns were silently
-- excluded from the list.
--
-- The correct attribution lives on `historic_order_lines.campaign_id`
-- (set during the historic import). Joining through that surface
-- captures every historical campaign:
--   - In Search of Darkness 1995          — 6,206 orders / $573,877  (isod_orders)
--   - In Search of Darkness 80's          — 3,668 orders / $269,694  (indiegogo + kickstarter)
--   - In Search of the Last Action Heroes — 1,295 orders / $91,776   (indiegogo + kickstarter)
--   - The Thing Expanded                  —   129 orders / $6,286    (wix tail)
--
-- KPI totals (orders/customers/revenue) on Other Sources were already
-- correct — they sum historic_orders directly and include all three
-- "non-gumroad / non-shopify_legacy" platforms. Only the campaign
-- breakdown was wrong, so this migration replaces the campaigns CTE
...
begin
  ...
  -- ── THE FIX ─────────────────────────────────────────────────────
  -- Attribute historic orders to campaigns via historic_order_lines.campaign_id
  -- (NOT campaign_orders, which only carries live Shopify rows).

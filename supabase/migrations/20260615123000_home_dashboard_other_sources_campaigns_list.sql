-- Home dashboard: replace the "top products" list on the Other Sources
-- column with a "top campaigns" list. Wix + ISOD are historical channels —
-- the per-product breakdown isn't useful at this surface (most products
-- are bundle SKUs or one-off legacy codes), but the campaign attribution
-- is. Robin scans this column to see "what historical campaigns drove
-- these orders" rather than "what individual SKUs were bought".
--
-- Adds a new `campaigns` array on the shopify_legacy block. Keeps
-- `products` shape for wire compat (set to empty array — the frontend
-- branches on which the Other Sources column renders).
--
-- Shopify + Gumroad columns are untouched.
...
declare
  ...
  v_combined            jsonb;
begin
  -- ── Shopify (live + historic shopify_legacy) — unchanged ──────────
  ...
  select jsonb_agg(t.j) into v_shopify_products from (
    ...
    hist_p as (
      ...
      from aa_01_campaigns.historic_order_lines hol
      join aa_01_campaigns.historic_orders ho on ho.id = hol.historic_order_id
      where ho.order_status='paid' and ho.source_platform='shopify_legacy' and hol.product_name_raw is not null
  ...
  -- ── Gumroad (live + historic gumroad) — unchanged ─────────────────
  with live_orders as (
  ...
  -- ── Other Sources KPIs (Wix + ISOD) — unchanged from prior migration ──
  with wix_orders as (
    select count(distinct ho.id)::int as orders,
           count(distinct lower(ho.email))::int as customers,
           coalesce(sum(ho.gross_amount), 0)::numeric as revenue
    from aa_01_campaigns.historic_orders ho
    where ho.order_status='paid' and ho.source_platform NOT IN ('gumroad', 'shopify_legacy')
  ),
  wix_units as (
    select coalesce(sum(hol.quantity), 0)::int as u
    from aa_01_campaigns.historic_order_lines hol
  ...
  -- ── NEW: Other Sources top-campaigns list ────────────────────────
  -- Wix historic orders link to campaigns via campaign_orders (source +
  -- source_order_id join). ISOD orders carry campaign_id directly. Both
  -- get merged + grouped, then joined to campaigns for the display
  -- name. Orphan rows (orders not tied to any campaign — e.g. Wix
  -- imports that pre-date campaign tagging) are excluded from the list
  -- so the column shows real campaign attribution only.
  select jsonb_agg(t.j) into v_other_campaigns from (
    with wix_by_campaign as (
      select co.campaign_id,
             count(distinct ho.id)::int as orders,
             coalesce(sum(ho.gross_amount), 0)::numeric as revenue
      from aa_01_campaigns.historic_orders ho
      join aa_01_campaigns.campaign_orders co
        on co.source = ho.source_platform
       and co.source_order_id = ho.source_order_id
      where ho.order_status='paid'
    ...
    merged as (
      ...
      from (select * from wix_by_campaign union all select * from isod_by_campaign) u
  ...
  -- Still surfaced for the bar-chart visual on the Other Sources card.
  ...
  daily_wix as (
    ...
    where order_status='paid' and source_platform NOT IN ('gumroad', 'shopify_legacy')
  ...
  merged_daily as (
    select d, sum(c)::int as c from (select * from daily_wix union all select * from daily_isod) u group by d
  )
  select jsonb_agg(jsonb_build_object('date', days.d, 'count', coalesce(m.c, 0)) order by days.d)
  into v_other_timeline
  from days left join merged_daily m on m.d = days.d;

  -- Combined — canonical, unchanged.
  ...
  -- JSON shape: Other Sources keeps the `shopify_legacy` key (wire-compat)
  -- and gains a `campaigns` array. `products` is set to [] so the frontend
  -- TopProducts component renders empty if it accidentally still tries
  -- this column.

-- Dashboard vs Campaigns list revenue reconciliation.
--
-- Symptom: dashboard three-stream sum $12,421,519.39 vs campaigns list
-- snapshot sum $12,276,896.59 — gap of $144,622.80.
--
-- Root cause: build_home_dashboard_payload summed historic revenue
-- from historic_orders.gross_amount (order-level, includes Shopify
-- VAT + shipping), whereas the campaigns list aggregates
-- historic_order_lines.line_revenue (line-level, item-only) via
-- get_campaigns_historic_totals(). All non-Shopify historic imports
-- (Indiegogo / Kickstarter / Crowdox / Wix / historic Gumroad) have
-- line = gross so were drift-free; only Shopify + shopify_legacy carry
-- tax + shipping in gross_amount:
--   shopify (historic) drift: $94,250.17
--   shopify_legacy drift:     $50,372.63
--   total:                    $144,622.80
--
-- The recent C Chat imports (Aliens Expanded C2, Aliens Expanded
-- Digital, TerrorBytes C1+2 supplement) preserved this Shopify
-- convention, so the gap grew as more historic Shopify data landed.
--
-- Fix: dashboard sums historic_order_lines.line_revenue instead of
-- historic_orders.gross_amount in the shopify + other-sources blocks.
-- Post-fix gap = $136 (rounding across aggregate joins). Semantically
-- clean: "campaign revenue" is what customers paid for products; tax +
-- shipping aren't campaign-attributable.
--
-- Refreshes dashboard_snapshot so the / page shows the reconciled
-- numbers immediately.
...
begin
  ...
  hist_orders as (
    -- Line-level revenue (product only) instead of order gross (which
    -- includes Shopify VAT + shipping). Campaigns list uses the same
    -- source, so both surfaces now agree.
  ...
  with other_orders as (
    -- Same shift for the "Other Sources" bucket. Non-Shopify historic
    -- imports (Indiegogo / Kickstarter / Crowdox / Wix) already have
    -- line = gross so this is a no-op for them.
    select count(distinct ho.id)::int as orders,
           count(distinct lower(ho.email))::int as customers,
           coalesce(sum(hol.line_revenue), 0)::numeric as revenue
    from aa_01_campaigns.historic_orders ho

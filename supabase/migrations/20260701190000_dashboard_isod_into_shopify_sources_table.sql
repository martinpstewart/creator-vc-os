-- Dashboard reshape:
--   Shopify column now folds in ISOD (isod_orders + isod_order_lines).
--   ISOD orders were placed on the pre-launch Shopify checkout, so
--   they belong under Shopify semantically. Everything: KPI totals,
--   Top Products, 30-day timeline, customer counts.
--
--   Other Sources column drops ISOD and replaces the "Top campaigns"
--   table with a per-source breakdown (Kickstarter / Indiegogo /
--   Crowdox / Wix) so the panel reflects the sources that actually
...
begin
  ...
  isod_agg as (
    select count(distinct io.id)::int as orders,
           ...
           (select count(*)::int from aa_01_campaigns.isod_order_lines) as units
  ...
  select jsonb_build_object(
    'orders',    (select orders from live_order_revenue) + (select orders from hist_orders) + (select orders from isod_agg),
    'customers', (select count(*)::int from all_emails),
    'revenue',   (select revenue from live_order_revenue) + (select revenue from hist_orders) + (select revenue from isod_agg),
    'units',     (select units from live_order_revenue) + (select u from hist_units) + (select units from isod_agg)
  )
  into v_shopify_kpis;
  ...
  select jsonb_agg(t.j) into v_shopify_products from (
    ...
    isod_p as (
      select l.sku_after_correction as name, count(*)::int as units,
             coalesce(sum(l.price_paid), 0)::numeric as revenue, 0 as variant_count
      from aa_01_campaigns.isod_order_lines l
      where l.sku_after_correction is not null
    ...
    merged as (
      ...
      from (select * from live_p union all select * from hist_p union all select * from isod_p) u
      group by name
    )
  ...
  merged_daily as (
    ...
    from (select * from daily_live union all select * from daily_hist union all select * from daily_isod) u
    group by d
  )
  ...
  select jsonb_build_object(
    'orders',    (select orders from other_orders),
    'customers', (select customers from other_orders),
    'revenue',   (select revenue from other_orders),
    'units',     (select u from other_units)
  )
  into v_other_kpis;

  select jsonb_agg(t.j) into v_other_sources from (
    select jsonb_build_object(
    ...
    from (
      select ho.source_platform,
             count(distinct ho.id)::int as orders,
             count(distinct lower(ho.email))::int as unique_customers,
      ...
      group by ho.source_platform
      having coalesce(sum(hol.line_revenue), 0) > 0 or count(distinct ho.id) > 0
    ) x
  ...
  from days left join daily_other m on m.d = days.d;
  ...
  return jsonb_build_object(
    ...
    'shopify_legacy', v_other_kpis
                  ...
                  || jsonb_build_object('sources', coalesce(v_other_sources, '[]'::jsonb))
